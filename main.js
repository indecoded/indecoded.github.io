...
var colors = {1: "#1A237E",2: "#01579B",3: "#006064",4: "#004D40",5: "#00695C",6: "#00796B",7: "#1B5E20",8: "#2E7D32",9: "#388E3C",1019021840: "#33691E",10: "#558B2F",11: "#689F38",12: "#AFB42B",13: "#C0CA33",14: "#CDDC39",15: "#D4E157",16: "#FFEE58",17: "#FFEB3B",18: "#FDD835",19: "#FBC02D",20: "#F57F17",21: "#FF6F00",22: "#F57C00",23: "#FF9800",24: "#FFB74D",25: "#FF8A65",26: "#FF7043",27: "#FF5722",28: "#E64A19",29: "#D84315",30: "#BF360C"};
...
d3.select(".pagetitle").attr("x",870).attr("y",410).attr("text-anchor","middle");
d3.select(".amount").attr("x",870).attr("y",435).attr("text-anchor","middle");
d3.select(".total").attr("x",870).attr("y",447).attr("text-anchor","middle");
...
d3.json("iit.html", function(polygon14) {
	...
	while (k++ < (countries - 1)) {
		  ...
		  mostpoints = Math.max(line1.length, line2.length, line3.length, line4.length, line5.length, line6.length, line7.length, line8.length,line9.length,line10.length,line11.length,line12.length,line13.length,line14.length);
		  ...
		  polygon8.features[k].geometry.coordinates[0] = rotatearray(polygon8.features[k].geometry.coordinates[0], -rotation);
		  rotation = calcrotation(line1, line9);
		  polygon9.features[k].geometry.coordinates[0] = rotatearray(polygon9.features[k].geometry.coordinates[0], -rotation);		  
		  rotation = calcrotation(line1, line10);
		  polygon10.features[k].geometry.coordinates[0] = rotatearray(polygon10.features[k].geometry.coordinates[0], -rotation);
		  rotation = calcrotation(line1, line11);
		  polygon11.features[k].geometry.coordinates[0] = rotatearray(polygon11.features[k].geometry.coordinates[0], -rotation);
		  rotation = calcrotation(line1, line12);
		  polygon12.features[k].geometry.coordinates[0] = rotatearray(polygon12.features[k].geometry.coordinates[0], -rotation);		  
		  rotation = calcrotation(line1, line13);
		  polygon13.features[k].geometry.coordinates[0] = rotatearray(polygon13.features[k].geometry.coordinates[0], -rotation);
		  rotation = calcrotation(line1, line14);
		  polygon14.features[k].geometry.coordinates[0] = rotatearray(polygon14.features[k].geometry.coordinates[0], -rotation);
		  ...
		  polygon7.features[k].geometry.coordinates[0] = polygon7.features[k].geometry.coordinates[0].map(projection);
		   polygon8.features[k].geometry.coordinates[0] = polygon8.features[k].geometry.coordinates[0].map(projection);
		  polygon9.features[k].geometry.coordinates[0] = polygon9.features[k].geometry.coordinates[0].map(projection);
		  polygon10.features[k].geometry.coordinates[0] = polygon10.features[k].geometry.coordinates[0].map(projection);
		  polygon11.features[k].geometry.coordinates[0] = polygon11.features[k].geometry.coordinates[0].map(projection);
		  polygon12.features[k].geometry.coordinates[0] = polygon12.features[k].geometry.coordinates[0].map(projection);
		  polygon13.features[k].geometry.coordinates[0] = polygon13.features[k].geometry.coordinates[0].map(projection);
		  polygon14.features[k].geometry.coordinates[0] = polygon14.features[k].geometry.coordinates[0].map(projection);
...
var startradius=10;
	...
	function normal(){
			tempmap.data(polygon.features).transition().duration(2000).attr("d", pathn).style("fill","#F2E1AF");
			...
			d3.selectAll(".summary")
				.style("opacity",0);
			$(".linkhere").html("");	
			$(".txtbox").html('INDECODED is a data visualization project that uses open source data to illustrate India in a more striking manner than textual data. Humans are visual thinkers. Our brains can detect even minor aberrations in familiar images. <a href="https://en.wikipedia.org/wiki/Cartogram">Cartograms</a> present subtle information, usually overlooked by us.<br/> The application may take around 20 seconds to load, so kindly be patient for the interface to load.<br/><em>(NB: The map does not show the Union Territories and Telangana as the data was not available of the said regions in Census 2011.)</em>');
	}
	function literacy(){
			...
			d3.select(".amount")
				.text("74.04%");
			$(".linkhere").html('<a href="https://en.wikipedia.org/wiki/Indian_states_ranking_by_literacy_rate">Source</a>');
			$(".txtbox").html('Literacy rate is defined as the percentage of population above the age of seven that can read and write with understanding of at least one language. Kerala bags the first place in Literacy with a whopping 94%. The states that lag behind and hence shrink in the cartogram are one in the Centre and the East of India. It will be interesting to notice how the smaller states like Goa and Delhi swell up in performance indexii.');
		
			
	}
	function gdp(){
			tempmap.data(polygon3.features).transition().duration(2000).attr("d", pathn).style("fill", function(d){
	                   	return colors[d.properties.Rank];});
	...
	function pop(){
			...
			d3.select(".amount")
				.text("1.328 billion people");
			$(".linkhere").html('<a href="https://en.wikipedia.org/wiki/List_of_states_and_union_territories_of_India_by_population">Source</a>');
			$(".txtbox").html('India\'s population is heavily centered around the rivers as seen in this cartogram where the Gangetic Plains swell up because of housing 1/5th of the total Indian population. The relatively sparse north east and the state of Jammu and Kashmir, in contrast, shrinks down.');
	
	}
	function wealth(){
			tempmap.data(polygon5.features).transition().duration(2000).attr("d", pathn).style("fill", function(d){
	                   	return colors[d.properties.Rank];});

			d3.select(".pagetitle")
				.text("Average GDP Per Capital");
			d3.select(".amount")
				.text("INR 74,920");
			$(".linkhere").html('<a href="https://en.wikipedia.org/wiki/List_of_Indian_states_by_GDP_per_capita">Source</a>');
			$(".txtbox").html('The economic distribution in our country becomes more apparent on comparing the per capita GDP in which the smaller states seem to overtake the larger ones. Sikkim and Goa manifold their size while Uttar Pradesh shrinks to the size of Assam. ');

			
	}    
	function hdi(){
			tempmap.data(polygon6.features).transition().duration(2000).attr("d", pathn).style("fill", function(d){
	                   	return colors[d.properties.Rank];});
			
			d3.select(".pagetitle")
				.text("Average HDI Index");
			d3.select(".amount")
				.text("0.609");	
			$(".linkhere").html('<a href="https://en.wikipedia.org/wiki/List_of_Indian_states_and_territories_by_Human_Development_Index">Source</a>');
			$(".txtbox").html('Quoting Wikipedia, The Human Development Index (HDI) is a composite statistic of life expectancy, education, and income per capita indicators, which are used to rank countries into four tiers of human development. The country with the highest HDI is Norway with a value of 0.944. In our context, the best performing state, Kerala bags an HDI index of 0.79. There is a high degree of corelation between this cartogram and the GDP/Capita cartogram.');

	}
	function meda(){
			tempmap.data(polygon7.features).transition().duration(2000).attr("d", pathn).style("fill", function(d){
	                   	return colors[d.properties.Rank];});

			d3.select(".pagetitle")
				.text("Medals won in National Games 2015");
			d3.select(".amount")
				.text("1334");
			$(".linkhere").html('<a href="https://en.wikipedia.org/wiki/2015_National_Games_of_India">Source</a>');
			$(".txtbox").html('The 35th National Games of India was held in early 2015 in Kerala. There were 33 games played including Aquatics, Shooting, Canoeing, Wushu, Judo and so on. Kerala, Maharashtra and Haryana bagged the first three positions. The North overwhelmingly swells up indicating high participation in the games.');
		
	}
	function riots(){
			tempmap.data(polygon8.features).transition().duration(2000).attr("d", pathn).style("fill", function(d){
	                   	return colors[d.properties.Rank];});
	...
	function forests(){
			tempmap.data(polygon9.features).transition().duration(2000).attr("d", pathn).style("fill", function(d){
	                   	return colors[d.properties.Rank];});

			d3.select(".pagetitle")
				.text("National Forest Coverage");
			d3.select(".amount")
				.text("21.34%");
			$(".linkhere").html('<a href="https://en.wikipedia.org/wiki/Forest_cover_by_state_in_India">Source</a>');
			$(".txtbox").html('This cartogram has the most distortion amongst any of the other cartograms with the West left almost as a shadow to the mighty North East, boasting 50% of India\'s forest covers. ');
		
	}  	
	function vehicles(){
			tempmap.data(polygon10.features).transition().duration(2000).attr("d", pathn).style("fill", function(d){
	                   	return colors[d.properties.Rank];});

			d3.select(".pagetitle")
				.text("Vehicles per 1000 people");
			d3.select(".amount")
				.text("133");		
			$(".linkhere").html('<a href="https://en.wikipedia.org/wiki/Indian_states_ranked_by_number_of_vehicles">Source</a>');
			$(".txtbox").html('Not very surprisingly, smaller states like Goa and Delhi have the highest vehicles/people ratio, followed by the states in the North and then the south Indian states, which interestingly produce 36% of all motor vehicles in the country.');

					
	}  	
	function alc(){
			tempmap.data(polygon11.features).transition().duration(2000).attr("d", pathn).style("fill", function(d){
	                   	return colors[d.properties.Rank];});
	...
	function obesity(){
			...
			d3.select(".amount")
				.text("15.3%");		
			$(".linkhere").html('<a href="https://en.wikipedia.org/wiki/Obesity_in_India">Source</a>');
			$(".txtbox").html('Obesity is a rising phenomenon in the country. The central belt of India seems to be literally squeezed by the volumnous north and the south. Also, if you toggle the GDP/Capita cartogram, you shall observe a high corelation with this cartogram.');
	
	}  	
	function iit(){
			tempmap.data(polygon14.features).transition().duration(2000).attr("d", pathn).style("fill", function(d){
	                   	return colors[d.properties.Rank];});
